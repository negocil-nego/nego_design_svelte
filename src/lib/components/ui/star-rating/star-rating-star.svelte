<script lang="ts">
  ...
  let {
    ...
    class: className,
    state: _state,
  }: StarRatingStarProps = $props();

  const rating = useStarRatingContext();

  const fraction = $derived.by(() =>
    Math.min(Math.max(rating.value - index, 0), 1),
  );
  ...
  const displayState = $derived(
    isPartial ? "partial" : isActive ? "active" : "inactive",
  );
...
<span
  ...
  class={cn(
    "ring-ring text-primary ring-offset-background group/item size-3 rounded-md ring-offset-2 outline-hidden group-aria-disabled:opacity-50 focus-visible:ring-2",
    className,
  )}
  onclick={handleClick}
