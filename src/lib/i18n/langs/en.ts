export default {
    "language.en": "English",
    "language.pt": "Portuguese",
    "language.es": "Spanish",
    "language.fr": "French",
    "language.de": "German",
    "language.it": "Italian",
    "language.ja": "Japanese",
    "language.ko": "Korean",
    "language.zh": "Chinese",
    "language.ru": "Russian",
    "language.ar": "Arabic",

    "input.email.label": "Email",
    "input.email.placeholder": "Enter your email",
    "input.username.label": "Username",
    "input.username.placeholder": "Enter your username",
    "input.phone.label": "Phone",
    "input.phone.placeholder": "Enter your phone",
    "input.email-or-phone.label": "Email or Phone",
    "input.email-or-phone.placeholder": "Enter your email or phone",
    "input.username-or-email.label": "Username or Email",
    "input.username-or-email.placeholder": "Enter your username or email",
    "input.otp.label": "Verification Code",
    "input.otp.placeholder": "Enter the code",
    "input.password.label": "Password",
    "input.password.placeholder": "Enter your password",
    "input.password-confirm.label": "Confirm your password",
    "input.password-confirm.placeholder": "Enter your password",
    "input.name.label": "Name",
    "input.name.placeholder": "Enter your name",
    "input.birthday.label": "Birthday",
    "input.birthday.placeholder": "Select your birthday",
    "input.date.label": "Date",
    "input.date.placeholder": "Select the date",
    "input.date.interval.label": "Period",
    "input.date.start.label": "Start",
    "input.date.start.placeholder": "Start date",
    "input.date.end.label": "End",
    "input.date.end.placeholder": "End date",
    "input.gender.label": "Gender",
    "input.gender.placeholder": "Select your gender",
    "input.bio.label": "Bio",
    "input.bio.placeholder": "Describe your bio",

    "link.forget.password": "Forgot my password",
    "link.privacy-policy.label": "Privacy Policy",
    "link.terms-of-service.label": "Terms of Service",

    "label.or": "Or",
    "label.or.continue.with": "Or Continue With",
    "label.logout": "Log out",
    "label.next": "Next",
    "label.start": "Start",
    "label.help": "Help",
    "label.settings": "Settings",
    "label.panel": "Panel",
    "label.administrative": "Administrative",
    "label.platform": "Platform",
    "label.authentication": "Authentication",
    "label.create-account": "Create account",
    "label.forget-password": "Forgot Password",
    "label.reset-password": "Reset Password",
    "label.otp-verification": "Verify Code",
    "label.home": "Home",
    "label.manager": "Manager",
    "label.folder": "Folder",
    "label.file": "File",
    "label.users": "Users",
    "label.account": "Account",
    "label.profile": "Profile",
    "label.welcome": "Welcome",
    "label.back": "Back",
    "label.login": "Sign In",
    "label.register": "Register",
    "label.login.with": "Continue with {{name}}",
    "label.remember.me": "Remember me",
    "label.send": "Send",
    "label.search": "Search",
    "label.search.placeholder": "Do your research",
    "label.more.options": "More Options",
    "label.more.details": "More Details",
    "label.details": "Details",
    "label.view.profile": " View Profile",
    "label.description": "Description",
    "label.view.full": "View Full",
    "label.menu": "Menu",
    "label.action": "Action",
    "label.close": "Close",
    "label.navigation": "Navigation",
    "label.navigation.text": "Navigate through our application to access all features features and content",
    "label.promotion": "Promotions",
    "label.promotion.description": "Promotions imperdiveis",
    "label.highlight": "Highlights",
    "label.buy": "Buy",
    "label.submit": "Submit",
    "label.submitting": "Submitting...",
    "label.cancel": "Cancel",
    "label.security": "Security",
    "label.firstname": "Firstname",
    "label.lastname": "Lastname",
    "label.email": "Email",
    "label.phone": "Phone",
    "label.birthday": "Birthday",
    "label.address": "Address",
    "label.streetNumber": "Street number",
    "label.neighborhood": "Neighborhood",
    "label.state": "State/Province",
    "label.latitude": "Latitude",
    "label.longitude": "Longitude",
    "label.location": "Location",
    "label.location.get": "Get current location",
    "label.location.getting": "Getting location...",
    "label.social": "Social networks",
    "label.whatsapp": "WhatsApp",
    "label.facebook": "Facebook",
    "label.linkedin": "LinkedIn",
    "label.instagram": "Instagram",
    "label.tiktok": "TikTok",
    "label.error": "Error",
    "label.success": "Success",

    "admin.profile.title": "User data",
    "admin.profile.description": "Area dedicated to managing your personal profile. Check your registration data, update your contact information and keep your records up to date to receive important notifications and ensure fast, personalized support.",
    "admin.profile.tab.user.submit": "Save Profile",
    "admin.profile.tab.user.saving": "Saving...",
    "admin.address.title": "Address data",
    "admin.address.description": "Manage your business address. Fill in the street number, neighborhood and state/province. You can grab the latitude/longitude coordinates automatically using the current-location button.",
    "admin.address.tab.submit": "Save Address",
    "admin.address.tab.saving": "Saving...",
    "admin.social.title": "Social networks",
    "admin.social.description": "Add links to your social media profiles so users can reach you across different channels.",
    "admin.social.tab.submit": "Save Social Networks",
    "admin.social.tab.saving": "Saving...",

    "admin.security.title": "Security",
    "admin.security.description": "Area dedicated to your account's security. Change your password, email or phone, and manage sensitive options such as deactivating or deleting the account.",
    "admin.security.password.title": "Change password",
    "admin.security.password.description": "Update your password regularly to keep your account protected. You must provide the current password and confirm the new one.",
    "admin.security.password.submit": "Update Password",
    "admin.security.password.saving": "Updating...",
    "admin.security.password.current.label": "Current password",
    "admin.security.password.current.placeholder": "Enter current password",
    "admin.security.password.new.label": "New password",
    "admin.security.password.new.placeholder": "Enter new password",
    "admin.security.password.confirm.label": "Confirm new password",
    "admin.security.password.confirm.placeholder": "Confirm new password",
    "admin.security.email.title": "Change email",
    "admin.security.email.description": "Change the email associated with your account. To confirm the operation, a verification code (OTP) will be sent to the new email.",
    "admin.security.email.submit": "Change email",
    "admin.security.email.saving": "Changing...",
    "admin.security.email.current.label": "Current password",
    "admin.security.email.current.placeholder": "Enter current password",
    "admin.security.email.new.label": "New email",
    "admin.security.email.new.placeholder": "Enter new email",
    "admin.security.email.otp.label": "Verification code (OTP)",
    "admin.security.email.otp.note": "We will send a verification code to the new email to confirm the change.",
    "admin.security.phone.title": "Change phone",
    "admin.security.phone.description": "Change the phone number associated with your account. To confirm the operation, a verification code (OTP) will be sent to the new phone.",
    "admin.security.phone.submit": "Change phone",
    "admin.security.phone.saving": "Changing...",
    "admin.security.phone.current.label": "Current password",
    "admin.security.phone.current.placeholder": "Enter current password",
    "admin.security.phone.new.label": "New phone",
    "admin.security.phone.new.placeholder": "Enter new phone",
    "admin.security.phone.otp.label": "Verification code (OTP)",
    "admin.security.phone.otp.note": "We will send a verification code to the new phone to confirm the change.",
    "admin.security.danger.title": "Danger section",
    "admin.security.danger.description": "Irreversible actions on your account. Both actions require your current password.",
    "admin.security.danger.saving": "Processing...",
    "admin.security.danger.current.label": "Current password",
    "admin.security.danger.current.placeholder": "Enter current password",
    "admin.security.danger.deactivate.title": "Deactivate account",
    "admin.security.danger.deactivate.description": "Your account is suspended and your data is hidden from the public. You can reactivate it later from this page.",
    "admin.security.danger.deactivate.submit": "Deactivate",
    "admin.security.danger.delete.title": "Delete account",
    "admin.security.danger.delete.description": "Permanently deletes your account and all associated data. This action cannot be undone.",
    "admin.security.danger.delete.submit": "Delete",

    "text.login.welcome": "Welcome Back!",
    "text.login.subtitle": "Sign in to access your unified inbox",
    "text.login.continue": "Continue with Google or enter your details.",
    "text.login.no-account": "Don't have an account?",
    "text.login.sign-up": "Sign Up for free",
    "text.login.has-account": "Already have an account?",
    "text.login.sign-in": "Sign In",
    "text.login.users.count": "10 Million+ Users",
    "text.login.users.worldwide": "worldwide",
    "text.login": "Enter your credentials below to securely access your account and continue where you left off.",
    "text.register": "Fill in the fields below with your information to create your new account and enjoy all our features.",
    "text.register.welcome": "Create Your Account",
    "text.register.subtitle": "Join us today and start your journey",
    "text.register.continue": "Fill in the details below to get started.",
    "text.forget-password": "Enter your email address below and we will send you a link to reset your password.",
    "text.reset-password": "Enter the verification code sent to your email and create a new password.",
    "text.otp-verification": "Enter the verification code we sent to your email or phone.",

    "carousel.login.slide1.title": "Welcome to the Admin Panel",
    "carousel.login.slide1.description": "Manage your platform efficiently with our comprehensive administration tools. Monitor performance, configure settings, and oversee all operations from a single dashboard.",
    "carousel.login.slide2.title": "User Management",
    "carousel.login.slide2.description": "Create, edit, and manage user accounts with ease. Assign roles, set permissions, and ensure secure access control for your entire organization.",
    "carousel.login.slide3.title": "Analytics & Reports",
    "carousel.login.slide3.description": "Gain valuable insights with real-time analytics and detailed reports. Track key metrics, identify trends, and make data-driven decisions to grow your business.",

    "carousel.register.slide1.title": "Create Your Account",
    "carousel.register.slide1.description": "Join our community and unlock access to powerful features. Fill in your details below to get started on your journey.",
    "carousel.register.slide2.title": "Explore Our Features",
    "carousel.register.slide2.description": "Discover all the amazing features we offer. From secure transactions to real-time analytics, we've got you covered.",
    "carousel.register.slide3.title": "Welcome Aboard!",
    "carousel.register.slide3.description": "You're almost there! Complete your registration and unlock the full experience. Welcome to the community.",

    "carousel.forget-password.slide1.title": "Forgot Your Password?",
    "carousel.forget-password.slide1.description": "Don't worry! It happens to everyone. Enter your email address and we'll send you a link to reset your password securely.",
    "carousel.forget-password.slide2.title": "Check Your Inbox",
    "carousel.forget-password.slide2.description": "We've sent a password reset link to your email. Check your inbox and follow the instructions to create a new password.",
    "carousel.forget-password.slide3.title": "Create a New Password",
    "carousel.forget-password.slide3.description": "Choose a strong, unique password to keep your account secure. Make sure it's something memorable but hard for others to guess.",

    "carousel.reset-password.slide1.title": "Reset Your Password",
    "carousel.reset-password.slide1.description": "We've sent a verification code to your email. Enter it below along with your new password to regain access to your account.",
    "carousel.reset-password.slide2.title": "Create a Strong Password",
    "carousel.reset-password.slide2.description": "Choose a password that is at least 8 characters long with a mix of letters, numbers, and symbols for maximum security.",
    "carousel.reset-password.slide3.title": "You're All Set!",
    "carousel.reset-password.slide3.description": "Your password has been successfully reset. You can now sign in with your new credentials and continue where you left off.",

    "carousel.otp-verification.slide1.title": "Verify Your Identity",
    "carousel.otp-verification.slide1.description": "We've sent a one-time verification code to your email or phone. Enter it below to verify your identity and proceed.",
    "carousel.otp-verification.slide2.title": "Enter the Code",
    "carousel.otp-verification.slide2.description": "Check your inbox or phone messages for the 6-digit code. It may take a few moments to arrive.",
    "carousel.otp-verification.slide3.title": "Code Verified!",
    "carousel.otp-verification.slide3.description": "Your identity has been verified. You can now proceed with resetting your password or accessing your account.",

    "footer.subscribe": "Subscribe",
    "footer.subscribe.title": "Subscreva a nossa newsletter",
    "footer.subscribe.text": "Subscribe to our newsletter to receive the latest news, articles and resources sent directly to your inbox.",
    "footer.copyr": "All rights reserved",

    "profile-user.info.title": "Basic user info",
    "profile-user.info.firstName.label": "First name",
    "profile-user.info.firstName.placeholder": "John",
    "profile-user.info.lastName.label": "Last name",
    "profile-user.info.lastName.placeholder": "Doe",
    "profile-user.info.email.label": "Email",
    "profile-user.info.email.placeholder": "john@example.com",
    "profile-user.info.phone.label": "Phone",
    "profile-user.info.phone.placeholder": "+1 234 567 890",
    "profile-user.info.address.label": "Address",
    "profile-user.info.address.placeholder": "123 Main St, City",
    "profile-user.info.username.label": "Username",
    "profile-user.info.username.placeholder": "@johndoe",
    "profile-user.info.password.label": "Password",
    "profile-user.info.role.label": "Role",
    "profile-user.info.role.placeholder": "Administrator",

    "profile-user.resetPassword.title": "Change Password",
    "profile-user.resetPassword.current.label": "Current password",
    "profile-user.resetPassword.current.placeholder": "Enter current password",
    "profile-user.resetPassword.new.label": "New password",
    "profile-user.resetPassword.new.placeholder": "Enter new password",
    "profile-user.resetPassword.confirm.label": "Confirm new password",
    "profile-user.resetPassword.confirm.placeholder": "Confirm new password",
    "profile-user.resetPassword.submit": "Update Password",
    "profile-user.resetPassword.cancel": "Cancel",

    "profile-user.setting.notification.title": "Email/Notification preferences",
    "profile-user.setting.notification.email.label": "Email notifications",
    "profile-user.setting.notification.email.desc": "Receive updates via email",
    "profile-user.setting.notification.web.label": "Web notifications",
    "profile-user.setting.notification.web.desc": "Receive updates on the web",
    "profile-user.setting.notification.app.label": "App notifications",
    "profile-user.setting.notification.app.desc": "Receive updates via mobile app",
    "profile-user.setting.enable.title": "Enable notification",
    "profile-user.setting.enable.email": "Email",
    "profile-user.setting.enable.web": "Web",
    "profile-user.setting.enable.app": "App",
    "profile-user.setting.marketing.title": "Marketing preferences",
    "profile-user.setting.marketing.type": "Type",
    "profile-user.setting.marketing.email": "Email",
    "profile-user.setting.marketing.push": "Push",

    "profile-user.variant1.tabs.basicInfo": "Basic user info",
    "profile-user.variant1.tabs.addresses": "Addresses",
    "profile-user.variant1.tabs.security": "Security",
    "profile-user.variant1.tabs.settings": "Settings",
    "profile-user.variant1.tabs.ecommerce": "E-Commerce",
    "profile-user.variant1.tabs.activity": "Activity logs",
    "profile-user.variant1.statusActive": "Active",
    "profile-user.variant1.roleDefault": "Customer",
    "profile-user.variant1.roleAdministrator": "Administrator",
    "profile-user.variant1.loginAsUser": "LOGIN AS USER",
    "profile-user.variant1.actionLoginAsUser": "LOGIN AS USER",
    "profile-user.variant1.addresses.title": "Shipping address",
    "profile-user.variant1.addresses.add": "Add",
    "profile-user.variant1.addresses.defaultBadge": "Default",
    "profile-user.variant1.addresses.empty": "No addresses added yet.",
    "profile-user.variant1.notes.title": "Notes",
    "profile-user.variant1.ecommerce.title": "E-Commerce",
    "profile-user.variant1.ecommerce.empty": "Order history and ecommerce details will appear here.",
    "profile-user.variant1.activity.title": "Activity logs",
    "profile-user.variant1.activity.empty": "User activity logs will appear here.",

    "profile-user.variant2.tabs.details": "Details",
    "profile-user.variant2.tabs.addresses": "Addresses",
    "profile-user.variant2.tabs.security": "Security",
    "profile-user.variant2.tabs.settings": "Settings",
    "profile-user.variant2.tabs.payment": "Payment",
    "profile-user.variant2.tabs.api": "API",
    "profile-user.variant2.tabs.profile": "Profile",
    "profile-user.variant2.banner.alt": "Banner",
    "profile-user.variant2.payment.title": "Payment Methods",
    "profile-user.variant2.payment.empty": "Saved payment methods and billing details will appear here.",
    "profile-user.variant2.api.title": "API Settings",
    "profile-user.variant2.api.empty": "API keys and integration settings will appear here.",

    "profile-user.variant3.tabs.about": "About",
    "profile-user.variant3.tabs.addresses": "Addresses",
    "profile-user.variant3.tabs.security": "Security",
    "profile-user.variant3.tabs.settings": "Settings",
    "profile-user.variant3.tabs.media": "Media",
    "profile-user.variant3.tabs.highlights": "Highlights",
    "profile-user.variant3.tabs.experience": "Experience",
    "profile-user.variant3.categories.performance": "Performance",
    "profile-user.variant3.categories.training": "Training",
    "profile-user.variant3.categories.education": "Education",
    "profile-user.variant3.categories.work": "Work",
    "profile-user.variant3.verifiedBadge": "Verified",
    "profile-user.variant3.experiences.emptyList": "No experiences found for this category.",
    "profile-user.variant3.experiences.emptySelection": "Select an experience to view details.",
    "profile-user.variant3.experiences.tags": "Tags",
    "profile-user.variant3.experiences.datePresent": "Present",
    "profile-user.variant3.experiences.descriptionPlaceholder": "Detailed description about this experience, including responsibilities, achievements and skills developed during this period.",
    "profile-user.variant3.media.title": "Media",
    "profile-user.variant3.media.empty": "Photos, videos and media gallery will appear here.",
    "profile-user.variant3.highlights.title": "Highlights",
    "profile-user.variant3.highlights.empty": "Achievements and highlights will appear here.",

    "empty.title": "No items found",
    "empty.description": "There are no data available to display at this time.",
    "empty.highlights.title": "No highlights available",
    "empty.highlights.description": "There are currently no highlighted items to display.",
    "empty.products.title": "No products available",
    "empty.products.description": "There are currently no products available to display.",
    "empty.promotions.title": "No active promotions",
    "empty.promotions.description": "There are currently no promotions available to display.",
    "empty.media.title": "No media found",
    "empty.media.description": "There are currently no photos or videos to display.",
    "empty.profiles.title": "No profiles found",
    "empty.profiles.description": "There are currently no profiles found to display.",
    "empty.not_found": "404 - Not Found",
    "empty.support": "Need help?",
    "empty.contact_support": "Contact support",

    "status.pending.title": "Pending",
    "status.pending.subtitle": "Your request is being processed",
    "status.pending.description": "This page will be automatically redirected or click the button below.",
    "status.pending.button": "Continue",
    "status.blocked.title": "Blocked",
    "status.blocked.subtitle": "Your access has been restricted",
    "status.blocked.description": "Please contact support to resolve this issue or click the button below.",
    "status.blocked.button": "Continue",
    "status.recovering.title": "Recovering Password",
    "status.recovering.subtitle": "We're working on resetting your password",
    "status.recovering.description": "Check your email for the recovery link. If you don't see it, check your spam folder or request a new link.",
    "status.recovering.button": "Back to Login",
    "status.loading.title": "Loading",
    "status.loading.subtitle": "Your data is being prepared",
    "status.loading.description": "Please wait a moment. Do not close or refresh this page.",
    "status.loading.button": "Loading",

    "card.otp.email.subtitle": "Receive the code by email",
    "card.otp.phone.subtitle": "Receive the code by SMS",
    "card.otp.whatsapp.subtitle": "Receive the code on WhatsApp",


    "search.input.placeholder": "Where are you going?",
    "search.checkin.label": "Check-in/check-out",
    "search.checkin.default": "Check-in - Check-out",
    "search.checkin.start": "Check-in",
    "search.checkin.end": "Check-out",
    "search.guests.label": "Guests and rooms",
    "search.guests.guest": "{{count}} guest",
    "search.guests.guests": "{{count}} guests",
    "search.guests.room": "{{count}} room",
    "search.guests.rooms": "{{count}} rooms",
    "search.guests.adults": "Adults",
    "search.guests.adults.age": "Age 13+",
    "search.guests.children": "Children",
    "search.guests.children.age": "Age 0-12",
    "search.guests.rooms.label": "Rooms",
    "search.guests.rooms.accommodation": "Accommodation",
    "search.guests.pets": "Pets allowed",
    "search.guests.pets.description": "Select to see stays that allow pets",
    "search.input.empty": "No results found",
    "search.input.no_results": "No results found",
    "search.input.try_another": "Try searching with other terms",
    "search.input.type_to_search": "Type to search"
};
